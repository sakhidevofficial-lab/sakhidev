...
export default function SplitSection() {
  ...
  useEffect(() => {
    const els = ref.current?.querySelectorAll('.reveal'); if (!els) return
    const io = new IntersectionObserver(entries => entries.forEach(e => { if (e.isIntersecting) { (e.target as HTMLElement).classList.add('visible'); io.unobserve(e.target) } }), { threshold: 0.08 })
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        <div className="reveal order-2 lg:order-1">
          ...
          <div className="flex flex-col">
            {steps.map((s) => (
              <div key={s.num} className="group flex gap-4 py-4 border-b border-gray-100 last:border-0 transition-all duration-200 hover:pl-2 cursor-default">
